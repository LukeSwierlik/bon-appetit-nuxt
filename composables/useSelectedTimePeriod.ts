...
import type { Period, SelectedTimePeriod } from '~/utils/common.interfaces';
import type { Ref } from 'vue';

export const useSelectedTimePeriod = (period: Ref<Period>) => {
  const current = computed<SelectedTimePeriod>(() => {
    switch (period.value) {
      case 'Yearly': {
        return {
  ...
  const previous = computed<SelectedTimePeriod>(() => {
    switch (period.value) {
      case 'Yearly': {
        return {
